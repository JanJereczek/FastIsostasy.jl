...
p = SolidEarthParameters(domain, layer_boundaries = lb, layer_viscosities = lv_3D)
...
tyr = t .* 1e3
sim = Simulation(domain, c, p, tyr, tyr, Hice_vec, output = "sparse", opts = opts)
run!(sim)
...
for k in eachindex(tplot)
    ...
    hidedecorations!(ax)
    heatmap!(ax, sim.out.u[kfi] + sim.out.ue[kfi]; opts...)
    println(kfi)
end
