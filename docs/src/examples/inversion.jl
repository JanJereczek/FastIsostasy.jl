...
Due to the large runtime of this computation, we do not produce the result for it through the online documentation build and invite you to try it out on your own machine. This remains an academic example, where we try to recover a known parameter field from data generated by the model itself. Nonetheless, the scheme can be extended to arbitrarily complex setups, where the data is obtained from observations or from a more complex model.
=#