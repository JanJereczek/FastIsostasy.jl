...
lb = get_layer_boundaries(Omega, T.(Tlitho), layering)
...
p = SolidEarthParameters(Omega, layer_boundaries = lb, layer_viscosities = T.(lv_3D))
...
it = TimeInterpolatedIceThickness(t .* 1f3, Hice_vec, Omega)
...
solve!(fip)
println(fip.nout.computation_time)
...
for k in eachindex(tplot)
    kfi = argmin( abs.(tplot[k] .- fip.nout.t) )
    ax = Axis(fig[1, k], aspect = DataAspect(), title = "t = $(fip.nout.t[kfi]) kyr")
    hidedecorations!(ax)
    heatmap!(ax, fip.nout.vals[:u][kfi] + fip.nout.vals[:ue][kfi]; opts...)
    println(kfi)
end
