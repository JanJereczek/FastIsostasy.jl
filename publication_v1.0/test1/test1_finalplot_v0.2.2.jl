...
function main()
    ...
    lwidth = 7
    msize = 35
    fig = Figure(resolution = (2700, 900), fontsize = ftsize)
    ...
    ylims!(ax4, (0, 10))
    barplot!(ax4, eachindex(t_plot), maxerror_t, label = L"max $\,$", color = :gray60)
    barplot!(ax4, eachindex(t_plot), meanerror_t, label = L"mean $\,$", color = :gray40)
    ...
    ax5 = Axis(fig[1, 3], title = L"(c) $\,$", xlabel = L"$N = N_{x} = N_{y} $ (1)",
    ...
    return maxerror_t, meanerror_t, maxerror, meanerror, delta_x, mmax, mmean
end

maxerror_t, meanerror_t, maxerror, meanerror, delta_x, mmax, mmean = main()