...
function mainplot(n)
    N = 2^n
    kernel = "cpu"
    suffix = "$(kernel)_Nx$(N)_Ny$(N)_dense_premparams"
    ...
    u_elva = get_denseoutput_fastiso("homogeneous_$(kernel)_Nx$(N)_Ny$(N)_dense.jld2",)
    ...
    figfile = "plots/test3/test3_v0.2_N=$(N)"
