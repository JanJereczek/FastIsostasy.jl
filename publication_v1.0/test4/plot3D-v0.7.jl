...
function load_1D_results(N)
    @load "../data/test4/ICE6G/1D-interactivesl=true-bsl=external"*
        "-N=$N.jld2" t fip
    return t, fip
end

function load_3D_results(case, N)
    @load "../data/test4/ICE6G/$case-N=$N.jld2" t fip
    return t, fip
end

function main(case, N; masktype="lgm")
    t, fip = load_3D_results(case, N)
    _, fip1D = load_1D_results(N)
    ...
    save("plots/test4/$case-N=$N-mask=$masktype-final_v0.7.png", fig)
    return nothing
end
