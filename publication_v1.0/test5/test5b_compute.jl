push!(LOAD_PATH, "../")
using JLD2, NCDatasets, CairoMakie, Interpolations
...
record(fig, "$dataset.gif", eachindex(t), framerate = 10) do k
    kobs[] = k
...
record(fig, "$dataset-displacement.gif", eachindex(t), framerate = 10) do k
    kobs[] = k
end
