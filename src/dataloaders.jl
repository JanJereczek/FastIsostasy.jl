...
function load_wiens2022(; extrapolation_bc = Throw())
    ...
    return (x, y, z),
    log10visc,
    linear_interpolation((x, y, z), log10visc, extrapolation_bc = extrapolation_bc)
end
...
function load_logvisc_pan2022()
    ...
    Interpolations.deduplicate_knots!(lat)
    logvisc_itp =
        linear_interpolation((lon, lat, r), logvisc, extrapolation_bc = Flat())
...
function load_latychev2023_ICE6G(; case = "1D", var = "R")
    ...
    for k in eachindex(latyfiles)
        X[:, :, k] = reshape(
            vec(readdlm(joinpath(latydir, latyfiles[k]))::Matrix{Float64}),
            nlon,
            nlat,
        )
    end
...
"""
function get_greenintegrand_coeffs(
    T::Type;
    file_is_remote = true,
